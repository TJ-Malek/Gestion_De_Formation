...
public interface SectionRepository extends JpaRepository<Section, Long>  {
	...
	@Query("select s from Section s where s.id_Chapitre  = ?1 order by s.date_ajout ASC")
	public List<Section> AllSectionChapitre(Long id_Chapitre);
	@Query("select s from Section s where s.id_Chapitre  = ?1 and s.etat=1 order by s.date_ajout ASC")
